  ...
  switch (type) {
    ...
    case 'review_received':
      return {
        ...
        heading: `¡Hola ${displayName}!`,
        message: `Has recibido una nueva reseña de <strong>${data.reviewerName}</strong> para <strong>${data.billboardTitle}</strong>.`,
        details: `
          <div style="background: rgba(155, 255, 67, 0.1); border-radius: 12px; padding: 16px; margin: 16px 0;">
            <p style="margin: 4px 0; color: #9BFF43; font-size: 18px;">${'⭐'.repeat(Number(data.rating))}</p>
    ...
    case 'welcome':
      return {
        subject: `¡Bienvenido a Maddi, ${displayName}!`,
        heading: `¡Hola ${displayName}!`,
        message: `Te damos la bienvenida a <strong>Maddi</strong>, la plataforma de publicidad exterior más grande de México.`,
        details: data.role === 'owner' 
          ? `
            <p style="color: rgba(255,255,255,0.8); font-size: 14px;">Como propietario, ahora puedes:</p>
            <ul style="color: rgba(255,255,255,0.6); font-size: 14px; padding-left: 20px;">
              <li>Publicar tus espacios publicitarios</li>
              <li>Recibir solicitudes de anunciantes</li>
              <li>Gestionar tus campañas y ganancias</li>
            </ul>
          `
          : `
            <p style="color: rgba(255,255,255,0.8); font-size: 14px;">Como anunciante, ahora puedes:</p>
            <ul style="color: rgba(255,255,255,0.6); font-size: 14px; padding-left: 20px;">
              <li>Explorar miles de espacios publicitarios</li>
              <li>Solicitar reservas directamente</li>
              <li>Gestionar tus campañas publicitarias</li>
            </ul>
          `,
        cta: data.role === 'owner' 
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #0A0A0A; padding: 40px 20px; margin: 0;">
  <div style="max-width: 480px; margin: 0 auto; background-color: #1A1A1A; border-radius: 16px; padding: 40px; border: 1px solid rgba(255,255,255,0.1);">
    ...
    <div style="text-align: center; margin-bottom: 24px;">
      <h1 style="color: #FFFFFF; font-size: 22px; margin: 0 0 8px 0;">${content.heading}</h1>
    </div>
    
    <div style="color: rgba(255,255,255,0.8); font-size: 14px; line-height: 1.6; margin-bottom: 24px;">
      ${content.message}
    </div>
    ...
    ${content.cta ? `
      <a href="${content.cta.url}" style="display: block; background-color: #9BFF43; color: #1A1A1A; text-decoration: none; padding: 14px 24px; border-radius: 12px; font-weight: 600; text-align: center; font-size: 15px; margin-top: 24px;">
        ${content.cta.text}
      </a>
    ` : ''}
