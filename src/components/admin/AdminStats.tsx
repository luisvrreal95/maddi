...
const AdminStats = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const completedCampaigns = bookings?.filter(b => 
          b.status === 'completed'
        ).length || 0;
        ...
        // Fetch pending payouts from commissions
        const { data: pendingCommissions } = await supabase
          .from('platform_commissions')
          .select('owner_payout')
          .eq('payment_status', 'pending');

        const pendingPayouts = pendingCommissions?.reduce((sum, c) => 
          sum + Number(c.owner_payout), 0
        ) || 0;

        setStats({
          totalOwners: ownerRoles?.length || 0,
          totalBusinesses: businessRoles?.length || 0,
          totalBillboards: billboardCount || 0,
          activeCampaigns,
          completedCampaigns,
          totalRevenue,
  ...
  const statCards = [
    ...
    {
      title: 'Comisión Maddi (15%)',
      value: formatCurrency(stats?.maddiCommission || 0),
      icon: TrendingUp,
