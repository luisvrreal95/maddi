        ...
        {images.length === 1 ? (
          ...
          <div className="grid grid-cols-2 gap-1 h-[300px] md:h-[420px]">
            {images.slice(0, 2).map((img, i) => (
              <img key={i} src={img} alt={`${title} ${i + 1}`} onClick={() => openLightbox(i)}
                className="w-full h-full object-cover hover:opacity-90 transition-opacity" />
            ))}
          </div>
        ) : images.length === 3 ? (
          <div className="grid grid-cols-2 gap-1 h-[300px] md:h-[420px]">
            <img src={images[0]} alt={title} onClick={() => openLightbox(0)}
              className="w-full h-full object-cover hover:opacity-90 transition-opacity row-span-2" />
            <div className="grid grid-rows-2 gap-1">
              {images.slice(1, 3).map((img, i) => (
                <img key={i} src={img} alt={`${title} ${i + 2}`} onClick={() => openLightbox(i + 1)}
                  className="w-full h-full object-cover hover:opacity-90 transition-opacity" />
              ))}
            </div>
          </div>
